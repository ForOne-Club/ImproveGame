...
//        HashSet<int> favIds = [];
//        // [AI注释] 先把收藏的 Buff 放在列表前面，并记录 ID 以避免后续重复添加。
...
//        // [AI注释] 再追加其余已启用隐藏的 Buff，保持“收藏优先、普通在后”的顺序。
...
//        {
//            // [AI注释] 用滚动条偏移驱动内部列表位置，UIList 本身不直接处理该滚动条。
...
//        // [AI注释] 通过 Buff 数量变化触发重建列表，避免每帧都重新生成按钮。
...
//        // [AI注释] 右键在“收藏/取消收藏”间切换，并立即刷新列表顺序与星标状态。
//        if (_isFavorited)
//        {
...
//        // [AI注释] 根据当前状态动态拼接交互提示文案（左键开关、右键收藏）。
//        if (buffEnabled)
