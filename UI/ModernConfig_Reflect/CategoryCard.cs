...
public partial class ModernReflectConfigUI
{
    public sealed class CategoryCard : TimerView
    {
        ...
        public CategoryCard(ModConfig config)
        {
            Config = config;
            ...
            _texture = TextureAssets.Item[ItemID.WireKite].Value;//category.GetIcon();

            OnMouseOver += (_, _) => SoundEngine.PlaySound(SoundID.MenuTick);
            OnLeftMouseDown += LeftMouseDownEvent;

            _labelElement = new SlideText(config.DisplayName.Value, 54)
            {
                VAlign = 0.5f,
                Left = { Pixels = 44 },
        ...
        {
            ConfigOptionsPanel.CurrentConfig = Config;
        }
        ...
        public override void DrawSelf(SpriteBatch spriteBatch)
        {
            ...
            // 及时更新文字，用于切换语言的情况
            if (Config.DisplayName.Value != _labelElement.DisplayText)
                _labelElement.DisplayText = Config.DisplayName.Value;
            // 英文长一点，因此缩小字体
            ...
            if (ConfigOptionsPanel.CurrentConfig == Config)
                _selectTimer.Open();
            else
                _selectTimer.Close();
