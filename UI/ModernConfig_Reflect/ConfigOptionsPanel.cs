...
public partial class ModernReflectConfigUI
{
    public sealed class ConfigOptionsPanel : SUIPanel
    {
        ...
        private static ModConfig _currentConfig;
        private static ModConfig ConfigToSelectOnOpen;
        ...
        {
            ModConfig config = _currentConfig;
            foreach (PropertyFieldWrapper variable in ConfigManager.GetFieldsAndProperties(_currentConfig))
            {
                ...
                if (type == typeof(bool))
                {
                    AddToggle(config, name);
                }
                else if (type == typeof(float) || type == typeof(int) || type == typeof(double))
                {
                    AddValueSlider(config, name);
                }
                else if (type.IsEnum)
                {
                    AddEnum(config, name);
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (ConfigToSelectOnOpen is not null)
            {
                CurrentConfig = ConfigToSelectOnOpen;
                ConfigToSelectOnOpen = null;
            }
        }
