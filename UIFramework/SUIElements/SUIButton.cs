...
public class SUIButton : TimerView
{
    ...
    public override void DrawSelf(SpriteBatch spriteBatch)
    {
        ...
        var list = ChatManager.ParseMessage(_text, TextColor);
        ChatManager.ConvertNormalSnippets(list);
        var textScale = new Vector2(TextScale);

        if (TextHasBorder)
        {
            ChatManager.DrawColorCodedStringShadow(spriteBatch, font, list, textPos,
                new Color(0, 0, 0, TextColor.A), 0f, Vector2.Zero, textScale, spread: 1f);
        }

        ChatManager.DrawColorCodedString(spriteBatch, font, list, textPos, Color.White, 0f, Vector2.Zero, textScale, out int _, -1f);
