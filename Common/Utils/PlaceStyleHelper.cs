        ...
        public static int TileFrameToPlaceStyle(int type, int frameX, int frameY)
        {
            switch (type)
            {
                ...
                case TileID.MusicBoxes: // 竖向排列 两格间隙 双排
                    if (frameX is 0 or 16 * 2)
                        return frameY / 54;
                    else if (frameX is 16 * 4 or 16 * 6)
                        return frameY / 54 + 56;
                    return 0;
                case TileID.Chandeliers: // 竖向排列 三格间隙 双排
                    if (frameX is 0 or 16 * 3)
                        return frameY / 54;
                    else if (frameX is 16 * 6 or 16 * 9)
                        return frameY / 54 + 37;
                    return 0;
                ...
                case TileID.ClosedDoor:
                    if (frameX is 0 or 16 or 16 * 2)
                        return frameY / 54;
                    else if (frameX is 16 * 3 or 16 * 4 or 16 * 5)
                        return frameY / 54 + 36;
                    return 0;
                case TileID.Lamps:
                    if (frameX is 0 or 16 * 2)
                ...
                case TileID.Banners: // 横向 一格间隔
                    if (frameY is 0)
                        return frameX / 16;
                    else if (frameY is 16 * 3)
                        return frameX / 16 + 111;
                    else
                        return frameX / 16 + 222;
