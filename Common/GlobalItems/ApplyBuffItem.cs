...
namespace ImproveGame.Common.GlobalItems
{
    ...
    {
        // 特殊药水
        public static readonly HashSet<int> SpecialPotions = new() {
            ItemID.RecallPotion,
        ...
        public override void ModifyTooltips(Item item, List<TooltipLine> tooltips)
        {
            ...
            {
                HideBuffSystem.PotionCount.TryGetValue(item.type, out int count);
                Color color = HideBuffSystem.InfiniteBuff.Contains(item.buffType) ? new Color(25, 255, 25) : new Color(255, 25, 25);
                string tip = GetTextWith(HideBuffSystem.InfiniteBuff.Contains(item.buffType) ? "Tips.TagDetailed.OwnedInfineBuff" : "Tips.TagDetailed.NotOwnedInfineBuff", new { Count = count, Threshold = Config.NoConsume_PotionRequirement });
                tooltips.Add(new(Mod, "HasItemBuff", $"{tip}") { OverrideColor = color });
            }
