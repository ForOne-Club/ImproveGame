...
namespace ImproveGame.Common.Players
{
    public class InfBuffPlayer : ModPlayer
    {
        ...
        {
            HideBuffSystem.InfiniteBuff.Clear();
            HideBuffSystem.PotionCount.Clear();
        ...
        public static void HandleBuffItem(Item item)
        {
            ...
            if (item.buffType > 0)
            {
                HideBuffSystem.InfiniteBuff.Add(buffType);
                if (HideBuffSystem.PotionCount.ContainsKey(item.type))
                {
                    HideBuffSystem.PotionCount[item.type] += item.stack;
                }
                else
                {
                    HideBuffSystem.PotionCount.Add(item.type, item.stack);
                }
            }
